...
import numpy as np
from plaspix.spixel_utils import *
from plaspix.ssn import CNN
import os, argparse
...
def members_from_clusters(sigma_val_xy, sigma_val_cnn, XY_features, CNN_features, clusters):
    ...
    # code to reduce memory footprint in cuda by deleting unwanted variables on cuda
    del sigma_array_xy, sigma_array_cnn
    del clusters_xy, dist_sq_xy, clusters_cnn, dist_sq_cnn
...
def optimize_spix(criterion, optimizer, scheduler, norm_val_x, norm_val_y, num_iterations=1000, image_width=None, image_height=None):
...
def plot_propagated(save_path, propagated, NUM_CLASSES):
    ...
    if NUM_CLASSES == 35:
        ...
        logger.error("We don't have a stored colour map for that quantity of classes, please specify - see the plot propagated function.")
    ...
    if cmap_exists('my_colormap') is False:
...
def propagate_labels(args):
    ...
    parser.add_argument('-s', '--num_spixels', action='store', type=int, default=100, dest='num_spixels', help='number of superpixels to be intialized for given image')
    ...
    if points == False:
        pa_metric = torchmetrics.Accuracy(task="multiclass", num_classes = NUM_CLASSES, ignore_index=unlabeled)
        mpa_metric = torchmetrics.Accuracy(task="multiclass", num_classes = NUM_CLASSES, ignore_index=unlabeled, average='macro')
        iou_metric = torchmetrics.JaccardIndex(task="multiclass", num_classes = NUM_CLASSES, ignore_index=unlabeled, average='none')

    logger.debug("setup is complete, now iterating through your images...")

    ### Iterate through the specified images ###
    with tqdm(images_filtered) as tqdm_images_filtered:
        for image_name in tqdm_images_filtered:
            tqdm_images_filtered.set_postfix(image=f"{image_name}")
            ...
            if ensemble:
                ...
                best_clusters_1 = optimize_spix(criterion_1, optimizer_1, scheduler_1,  norm_val_x=norm_val_x, norm_val_y=norm_val_y, num_iterations=num_iterations, image_width=image_width, image_height=image_height)
                best_members_1 = members_from_clusters(sigma_xy_1, sigma_cnn_1, XY_features, CNN_features, best_clusters_1)

                best_clusters_2 = optimize_spix(criterion_2, optimizer_2, scheduler_2, norm_val_x=norm_val_x, norm_val_y=norm_val_y, num_iterations=num_iterations, image_width=image_width, image_height=image_height)
                best_members_2 = members_from_clusters(sigma_xy_2, sigma_cnn_2, XY_features, CNN_features, best_clusters_2)

                best_clusters_3 = optimize_spix(criterion_3, optimizer_3, scheduler_3, norm_val_x=norm_val_x, norm_val_y=norm_val_y, num_iterations=num_iterations, image_width=image_width, image_height=image_height)
                ...
                best_clusters = optimize_spix(criterion, optimizer, scheduler,  norm_val_x=norm_val_x, norm_val_y=norm_val_y, num_iterations=num_iterations, image_width=image_width, image_height=image_height)
            ...
            if save_rgb is not None:
                plot_propagated(os.path.join(save_rgb, image_name[:-4]), propagated_full, NUM_CLASSES)
            ...
            # current image has been processed by the time code reaches here
            # delete variables that are no longer needed for next image to reduce memory footprint
            del sparse_labels, XYLab, X, Y
            del features, features_magnitude_mean, features_rescaled, features_cat, XY_cat
            del mean_init, CNN_features, XY_features
            del propagated_as_image

            if points == False:
                del sparse_mask
                del criterion_1, optimizer_1, scheduler_1
                del criterion_2, optimizer_2, scheduler_2
                del criterion_3, optimizer_3, scheduler_3
                del best_clusters_1, best_clusters_2, best_clusters_3                
                del best_members_1_max, best_members_2_max, best_members_3_max
                del connected_1, connected_2, connected_3
                del prop_1, prop_2, prop_3, prop_count
                del propagated_torch, labels_torch, propagated_full
            else:
                del criterion, optimizer, scheduler, best_clusters, best_members, connected, propagated_full

            # Clear the cache before the next image
            torch.cuda.empty_cache()

    # all images have been processed by the time code reaches here
    # delete variables that are no longer needed to reduce memory footprint
    del xylab_function, spixel_centres
    del pil_img, GT_pil_img, image_height, image_width, 
    del image, GT_mask_np, GT_mask, GT_mask_torch
    del model_dict, obj, pretrained_dict, CNN_function

    logger.debug("propagation of point labels is complete!")

    # We can only evaluate our propagation if we have dense ground truth to compare it to
    if points == False:
        logger.debug("evaluation script working...")
        acc = pa_metric.compute()
        m_acc = mpa_metric.compute()
        miou = iou_metric.compute()

        logger.info(f"per class mean intersection over union: {miou}")
        ...
        evaluation_metrics = \
            dict(
                PA=acc.item()*100, 
                mPA=m_acc.item()*100, 
                mIO_per_class= mean_iou_torch.item()*100
                )
        # print("PA:", acc.item()*100, ", mPA:", m_acc.item()*100, ", mIOU per class:", mean_iou_torch.item()*100)
        logger.info(f"Evaluation Metrics: {evaluation_metrics}")

        # delete variables that are no longer needed to reduce memory footprint
        del pa_metric, mpa_metric, iou_metric
        del acc, m_acc, miou, class_ious_torch, mean_iou_torch, evaluation_metrics


if __name__ == '__main__':
    propagate_labels(sys.argv[1:])